...
function uiChord(freqs, gain, atk, dec, spread=0) {
  if (!ensureUICtx()) return;
  const now = uiCtx.currentTime;
  freqs.forEach((f, i) => {
    const d = i * spread;
...
//  AMBIENT AUDIO
// ══════════════════════════════════════════
let audioCtx = null, masterGain = null, gainNode = null, audioPlaying = false, filterNode = null;
const oscillators = [];
...
function initAudio() {
  ...
  comp.threshold.value=-18; comp.knee.value=20; comp.ratio.value=4; comp.attack.value=0.08; comp.release.value=0.5;
  ...
  const d0=makeDelay(.29,.12), d1=makeDelay(.47,.10), d2=makeDelay(.71,.08), d3=makeDelay(1.13,.06);
  ...
  layers.forEach(([f1,f2,vol,pan,lfoF,lfoA,delay]) => {
    [f1,f2].forEach((f,j) => {
      const osc=ctx.createOscillator(), g=ctx.createGain(), p=ctx.createStereoPanner(), lp=ctx.createBiquadFilter();
      ...
      g.gain.setValueAtTime(0,now+delay); g.gain.setTargetAtTime(v,now+delay,4.5+j*.5);
...
function playHoverTone(key) {
  ...
  c.freqs.forEach((f,i) => {
    const o=audioCtx.createOscillator(), g=audioCtx.createGain(), lp=audioCtx.createBiquadFilter();
    o.type='sine'; o.frequency.value=f; lp.type='lowpass'; lp.frequency.value=f*4; lp.Q.value=.5;
    const vol=c.gain*(i===1?.55:1);
    g.gain.setValueAtTime(0,now); g.gain.linearRampToValueAtTime(vol,now+c.atk); g.gain.exponentialRampToValueAtTime(.0001,now+c.atk+c.dec);
    o.connect(lp); lp.connect(g); g.connect(masterGain); o.start(now); o.stop(now+c.atk+c.dec);
  });
}

function playActivationSound(key) {
  if (!audioCtx || !audioPlaying) return;
  const now = audioCtx.currentTime;
  const cfg = {
  ...
  c.freqs.forEach((f,i) => {
    ...
    g.gain.setValueAtTime(0,now+d); g.gain.linearRampToValueAtTime(vol,now+d+c.atk); g.gain.exponentialRampToValueAtTime(.0001,now+d+c.atk+c.dec);
...
const _uiLast = {};
const _uiBase = uiSound;
uiSound = function(type, ms=160) {
  const n=performance.now();
  if (_uiLast[type] && n-_uiLast[type] < ms) return;
  _uiLast[type]=n; _uiBase(type);
};
...
function playIntroSound() {
  ...
  comp.threshold.value=-12; comp.knee.value=18; comp.ratio.value=5; comp.attack.value=.02; comp.release.value=.4;
  comp.connect(uiCtx.destination);
  [55,82.41,110].forEach((f,i) => {
    const o=uiCtx.createOscillator(),g=uiCtx.createGain(),lp=uiCtx.createBiquadFilter();
    o.type='sine'; o.frequency.value=f; lp.type='lowpass'; lp.frequency.value=f*5; lp.Q.value=.4;
    const d=i*.18;
    g.gain.setValueAtTime(0,now+d); g.gain.linearRampToValueAtTime(.055-i*.01,now+d+1.2); g.gain.setTargetAtTime(.022,now+3.0,1.0);
    o.connect(lp); lp.connect(g); g.connect(comp); o.start(now+d); o.stop(now+12);
  });
  const dotFreqs=[523.2,587.3,659.2,698.5,783.9,880], dotDelays=[1.0,1.4,1.7,2.0,2.3,2.6];
  dotFreqs.forEach((f,i) => {
    const d=dotDelays[i], o=uiCtx.createOscillator(),g=uiCtx.createGain(),lp=uiCtx.createBiquadFilter();
    o.type='sine'; o.frequency.value=f; lp.type='lowpass'; lp.frequency.value=f*3.5; lp.Q.value=.6;
    g.gain.setValueAtTime(0,now+d); g.gain.linearRampToValueAtTime(.068,now+d+.04); g.gain.exponentialRampToValueAtTime(.0001,now+d+1.5);
    o.connect(lp); lp.connect(g); g.connect(comp); o.start(now+d); o.stop(now+d+1.5);
  });
  [1320,1760,2200].forEach((f,i) => {
    const d=2.35+i*.07,o=uiCtx.createOscillator(),g=uiCtx.createGain(),lp=uiCtx.createBiquadFilter();
    o.type='sine'; o.frequency.value=f; lp.type='lowpass'; lp.frequency.value=f*1.8; lp.Q.value=1.2;
    g.gain.setValueAtTime(0,now+d); g.gain.linearRampToValueAtTime(.052-i*.012,now+d+.06); g.gain.exponentialRampToValueAtTime(.0001,now+d+2.4);
    o.connect(lp); lp.connect(g); g.connect(comp); o.start(now+d); o.stop(now+d+2.4);
  });
}

// Mouse reactive audio — modulates filter cutoff on fast mouse moves
let lastMouseX=0, lastMouseY=0;
document.addEventListener('mousemove', e => {
  if (!entered||!audioCtx||!audioPlaying||!filterNode) return;
  const dx=e.clientX-lastMouseX, dy=e.clientY-lastMouseY;
  lastMouseX=e.clientX; lastMouseY=e.clientY;
  const vel=Math.min(Math.sqrt(dx*dx+dy*dy),40);
  if (vel>3) {
    // FIX: use AUDIO_CUTOFFS instead of duplicated inline object
    const b = AUDIO_CUTOFFS[activeKey] ?? 800;
...
function playCTAHover(variant) { uiSound('hover:panel-cta-'+variant); }
function playCardHover()        { uiSound('hover:pm-card'); }
function openProject(slug)      { console.log('openProject:', slug); }
...
function returnToIntro() {
  closePanel();
  pageFadeOut(() => {
    const hud=document.getElementById('hud'), labels=document.getElementById('labels');
    const intro=document.getElementById('intro'), content=document.getElementById('i-content');
    intro.classList.add('visible'); intro.style.display='flex'; intro.style.opacity='1';
    content.style.opacity='1'; content.style.transform=''; content.style.transition='';
    hud.classList.remove('on'); hud.style.opacity=''; hud.style.transition='';
    labels.innerHTML=''; labels.style.opacity=''; labels.style.transition='';
    entered=false; hoveredKey=null; activeKey=null;
    camTarget={x:0,y:0,z:580};
    if (audioCtx&&audioPlaying) gainNode.gain.setTargetAtTime(.25,audioCtx.currentTime,1.5);
...
// ══════════════════════════════════════════
function enter() {
  const intro=document.getElementById('intro'), trans=document.getElementById('enter-transition');
  const content=document.getElementById('i-content');
  content.style.transition='opacity .4s ease, transform .6s ease';
  content.style.opacity='0'; content.style.transform='scale(.97) translateY(-10px)';
  trans.classList.add('flash');
  if (audioCtx&&audioPlaying) {
    gainNode.gain.setTargetAtTime(.7, audioCtx.currentTime, 2.0);
  } else {
    initAudio();
    gainNode.gain.setTargetAtTime(.7, audioCtx.currentTime, 2.5);
    audioPlaying=true;
  }
  ...
  setTimeout(() => {
    pageFadeOut(() => {
      intro.style.display='none'; intro.classList.remove('visible');
      document.getElementById('hud').classList.add('on');
      initLabels();
      entered=true; trans.classList.remove('flash');
      pageFadeIn();
    }, 350);
  }, 2100);
}
...
function activateSplash() {
  ...
  splashDone = true;
  const splash=document.getElementById('splash'), intro=document.getElementById('intro');
  ensureUICtx();
  if (uiCtx && uiCtx.state==='suspended') uiCtx.resume();
  initAudio();
  gainNode.gain.setValueAtTime(0, audioCtx.currentTime);
  gainNode.gain.setTargetAtTime(.18, audioCtx.currentTime, 3.0);
  audioPlaying=true; audioPreEnabled=true;
  setTimeout(() => {
  ...
  pageFadeOut(() => {
    ...
    gainNode.gain.setTargetAtTime(.35, audioCtx.currentTime, 2.5);
  }, 500);
}
...
document.addEventListener('mousemove', e => {
  ...
  const dx=(e.clientX/innerWidth-.5)*18, dy=(e.clientY/innerHeight-.5)*10;
